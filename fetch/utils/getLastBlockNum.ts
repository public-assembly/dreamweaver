...
export async function getLastBlockNum() {
  ...
  const etherscanApiUrl = `${env.ETHERSCAN_ENDPOINT}/api?module=contract&action=getcontractcreation&contractaddresses=${env.CONTRACT_ADDRESS}&apikey=${env.OPTIMISTIC_ETHERSCAN_API_KEY}`;
